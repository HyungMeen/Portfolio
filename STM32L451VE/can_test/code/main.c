...
uint32_t TxMailbox;
uint8_t TxData[8]={11,22,33,44,55,66,77,88};
uint8_t RxData[8]={0,};
uint8_t RxcoData[8]={11,22,33,44,55,66,77,88};
/* USER CODE END 0 */
...
static void MX_CAN1_Init(void)
{
  ...
  /*if (HAL_CAN_ActivateNotification(&hcan1,CAN_IT_RX_FIFO0_FULL|CAN_IT_RX_FIFO0_MSG_PENDING)!=HAL_OK)
  {
	  Error_Handler();
  }*/
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
	if(hcan->Instance == CAN1)
	{
		...
		if (RxData[0]>0)
		{
			...
			if (RxHeader.DLC > 0)
			        {
			            if (RxHeader.DLC == 8 && memcmp(RxData, RxcoData, 8u) == 0)
			            {
			            	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, SET);
			            }
			            else
